...
const SuccessStep = ({ fileToUpdate, handleCloseDialog, history, seconds }) => {
  return (
    <Segment placeholder>
      ...
      <Segment.Inline>
        <Button primary onClick={() => { history.push(`${window.location.pathname}/${fileToUpdate.kfId}`) }} >View Document</Button>
        <Button onClick={() => { handleCloseDialog() }}>Exit ({3 - seconds})</Button>
      </Segment.Inline>
    </Segment>
  )
}

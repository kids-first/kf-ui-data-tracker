...
const uploadedFile = new File([''], 'organize.tsv', { type: "text/tab-separated-values", lastModified: new Date(0) });
const historyMock = { push: jest.fn(), location: { pathname: '/study/SD_8WX8QQ06/documents' } };
let file = { lastModified: 1567797130510, name: 'organize.tsv', size: 7, type: "text/tab-separated-values", webkitRelativePath: "" }
const sleep = (ms) =>
...
it('progresses to Step 3: Document Updated ', async (done) => {
  ...
  const { container, getByText, getByTestId, debug } = render(
    ...
    <MockedProvider mocks={mocks}>
      <MemoryRouter
        initialEntries={['/study/SD_8WX8QQ06/documents']}
      >
        <UploadWizard
          history={historyMock}
          studyId={studyByKfId.data.kfId}
  ...
  act(() => {
    fireEvent.change(getByTestId('description-input'), { target: { value: inputVal } })
  })
